...
import { fetchCulture, SimpleItem } from '../lib/contentApi';
...
const cultureStories = [
  { title:'Pahela Baishakh', subtitle:'Bengali New Year', image:'/images/culture/pahela-baishakh.jpg', detail:'Celebrated on 14 April with colourful processions, fairs, music, traditional dress and seasonal foods.' },
  { title:'Jamdani Weaving', subtitle:'Living textile heritage', image:'/images/culture/jamdani-weaving.jpg', detail:'The delicate motifs of Jamdani are handwoven by skilled artisans and represent one of Bengal’s most celebrated textile traditions.' },
  { title:'Bengali Cuisine', subtitle:'Flavours of river and delta', image:'/images/culture/bengali-cuisine.jpg', detail:'Rice, fish, vegetables, bhorta, pitha and sweets create a cuisine shaped by seasons, rivers and regional ingredients.' },
  { title:'Baul Music', subtitle:'Songs of the wandering mystics', image:'/images/culture/baul-music.jpg', detail:'Baul songs combine music, poetry and spiritual philosophy and remain one of rural Bengal’s most distinctive folk traditions.' },
  { title:'Nakshi Kantha', subtitle:'Stories stitched by hand', image:'/images/culture/nakshi-kantha.jpg', detail:'Decorative embroidered quilts turn everyday cloth into vivid stories of village life, nature and family memory.' },
  { title:'Nouka Baich', subtitle:'Traditional boat racing', image:'/images/culture/nouka-baich.jpg', detail:'Boat races animate rivers during festivals and monsoon seasons, combining teamwork, rhythm and intense local competition.' },
  { title:'Indigenous Traditions', subtitle:'Hill and plains communities', image:'/images/culture/indigenous-traditions.jpg', detail:'Chakma, Marma, Garo, Santal, Tripura and other communities enrich Bangladesh with distinct languages, textiles, foods and festivals.' },
  { title:'Rickshaw Art', subtitle:'Colour on the move', image:'/images/culture/rickshaw-art.jpg', detail:'Bright hand-painted motifs on rickshaws turn everyday transport into a distinctive form of popular urban art.' },
  { title:'Pitha Tradition', subtitle:'Seasonal cakes of Bengal', image:'/images/culture/pitha-tradition.jpg', detail:'Bhapa, patishapta, chitoi and many regional pithas are especially associated with winter, harvests and family gatherings.' },
  { title:'Jatra & Folk Theatre', subtitle:'Stories on the village stage', image:'/images/culture/jatra-folk-theatre.jpg', detail:'Open-air folk theatre combines music, dramatic storytelling and social themes in a performance tradition familiar across Bengal.' },
  { title:'Terracotta Heritage', subtitle:'Stories shaped in clay', image:'/images/culture/terracotta-heritage.jpg', detail:'Historic temples and monuments preserve intricate terracotta panels depicting floral forms, daily life and narrative scenes.' },
  { title:'Village Fairs', subtitle:'Mela and community life', image:'/images/culture/village-fairs.jpg', detail:'Seasonal melas bring together handicrafts, food, music, toys, performances and community celebration.' },
];
...
export default function Culture() {
  ...
  useEffect(() => { fetchCulture().then(setAdminItems).catch(() => setAdminItems([])); }, []);
  ...
  return <div className="max-w-6xl mx-auto px-6 py-12">
    ...
    {adminItems.length > 0 && (
      <div className="mt-16"><h2 className="text-2xl font-bold mb-6 text-heading">More Culture Stories</h2><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">{adminItems.map((item) => (
        <article key={item._id} className="bg-surface border border-line/10 rounded-2xl overflow-hidden hover:border-brand-green/30 transition-colors">
          {item.coverImage && <div className="h-48 overflow-hidden"><img src={item.coverImage} alt={item.title} loading="lazy" className="w-full h-full object-cover hover:scale-105 transition-transform duration-500"/></div>}
          <div className="p-5">{item.category && <p className="text-xs text-brand-green uppercase tracking-wider mb-1">{item.category}</p>}<h3 className="text-xl font-bold text-heading mb-2">{item.title}</h3><p className="text-sm text-muted leading-relaxed">{item.description}</p></div>
        </article>
      ))}</div></div>
    )}
  </div>;
